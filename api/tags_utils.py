...
def get_tagged_logs(logs):
    for i, log in enumerate(logs):
        sessions = log['sessions']
        for j, session in enumerate(sessions):
            logs[i]['sessions'][j]['requests'] = apply_tagging_old(
                session['requests'])
    return logs
...
def apply_tagging_old(requests):
    for i, request in enumerate(requests):
        request = request['request_url']
        pos1 = request.find('ark')
        pos2 = request.find('sru')
        ...
        if ('und' in request and '/accueil/' in request):
            if (pos4 > pos5):  # acceuil before und
                request = '##HomePage'+request
                requests[i]['tagged_request_url'] = request
                requests[i]['request_tag'] = 'HomePage'
            elif (pos5 > pos4):
                request = '##CollectionNavigation'+request
                requests[i]['tagged_request_url'] = request
...
def save_to_db(client_id, index, requests):
    field = "sessions."+str(index)+".requests"
    response = users_logs.update_one({'client_id': client_id}, {
                                     '$set': {field: requests}})
